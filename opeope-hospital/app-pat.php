...
<?php
    ...
    $hospNum = $userData['hospitalNum'];
    $sql3 = "SELECT * FROM Appointment WHERE hospitalNum='$hospNum'";
    $userHistory = mysqli_query($conn, $sql3);
?>
        ...
        <div style="overflow:auto; padding: 2.5% 2.5% 2.5% 2.5%; ">
            ...
            <div class="main">
                ...
                <div class="card rounded-0 shadow-sm" style="width: 100%; float: left; padding: 0px;">
                    <div style="padding: 10px 20px 10px 20px; border-bottom: 1px solid rgba(0, 0, 0, 0.1);background-color: #42b7b9; color: white;"><h4>Appointment List</h4></div>
                    <div style="min-height: 300px; overflow-y: hidden;">
                        <div style="margin-top: 0px; margin-bottom: 20px; margin-right: 20px; padding: 20px;">
                            ...
                            <div style="width: 100%; float: left; margin-bottom: 10px;">
                                <div style="width: 100%; font-weight: bold; text-align: center;">Appointment History</div>
                                <div style="width: 100%; margin: 10px; padding: 20px; border: 1px solid #CCCCCC; border-radius: 5px; overflow:scroll;">
                                    
                                    <div style="width: 100%;padding : 20px; height:100%;">
                                        <?php
                                        $i=1;
                                        if(!$row = mysqli_fetch_array($userHistory)){
                                        ?><div style="text-align: center;">ไม่พบประวัติการนัดหมาย</div>
                                        <?php    
                                        }
                                        while($row = mysqli_fetch_array($userHistory))
                                        {
                                            ?>
                                            <div style="width:70%; float: left;">
                                            <?php echo $i.") ".$row['History']; ?></div>
                                            <div style="width:25%; float: right; text-align: center;"><?php echo $row['AStatus']?></div><br><hr>
                                        <?php
                                        $i+=1;
                                        }
                                        ?>
