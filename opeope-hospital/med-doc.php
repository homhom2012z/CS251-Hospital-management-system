...
<?php
    ...
    function fill_name($conn){
        $output = '';
        $sqlx = "SELECT * FROM Medicine";
        $result = mysqli_query($conn, $sqlx);
        while($rowx = mysqli_fetch_array($result)){
            $output .= '<option value=">'.$row['MedicineID']. '">'. $rowx['TradeName']. '</option>';
        }

        return $output;
    }

    function fill_data($conn){
        $output = '';
        $sqlx2 = "SELECT * FROM Medicine";
        $result = mysqli_query($conn, $sqlx2);

        while($row = mysqli_fetch_array($result)){
            $output .= '<div class="col-md-3">';
            $output .= '<div style="border: 1px solid #ccc;">'.$row["TradeName"].'';
            $output .= '</div>';
            $output .= '</div>';
        }
        return $output;
        ...
        <div style="overflow:auto; padding: 2.5% 2.5% 2.5% 2.5%; ">
            <div class="menu" >
                <div>
                    ...
                    <a href="med.php"><div class="menuitem menu_active shadow-sm"><i class="fas fa-pills" style="padding-right: 7px;"></i>Medicine</div></a>
                    ...
                    <div style="min-height: 300px; overflow-y: hidden;">
                        <div style="margin-top: 0px; margin-bottom: 20px; margin-right: 20px; padding: 20px;">
                            ...
                            <div style="width: 48.5%; float: left; margin-bottom: 10px;">
                                <div style="width: 100%; font-weight: bold; text-align: center;">ตัวอย่าง</div>
                                <div style="width: 100%; margin: 10px; padding: 20px; border: 1px solid #CCCCCC; border-radius: 5px">
                                <div style="text-align: center;">
                                </div>
                                    <img src="assests/images/medicines/2.jpg" alt="medicine_1" width="500px">
                            ...
                            <div style="width: 50%; float: right; margin-bottom: 10px;">
                                ...
                                <form style="font-size: 16px;" action="med-doc_db.php" method="post">
                                    <div style="width: 100%; margin: 10px; padding: 20px; border: 1px solid #CCCCCC; border-radius: 5px">
                                        
                                            <div>
                                                <div style="margin-bottom: 10px;">Choose Patient</div>
                                                <select class="input-regis" name="PatientID" id="PatientID" required>
                                                <?php
                                            ...
                                            <div>
                                                <div style="margin-bottom: 10px;">Choose Medicine</div>
                                                <select class="input-regis" name="MedicineID" id="MedicineID" required>
                                                <?php
                                                while($row = mysqli_fetch_array($medicineList))
                                                {
                                                    ?>
                                                    <option value="<?php echo $row['MedicineID']?>"><?php echo $row['TradeName']; ?></option>
                                                <?php    
                                                }
                                                ?>
                                                </select>
                                            ...
                                            <div>
                                                ...
                                                <input class="input-regis" type="text" name="Description" placeholder="Description" required>
                                            </div>
                                        ...
                                        <div style="width: 100%; float: right; margin: 20px; text-align:">
                                            ...
                                            <div style="width:30%; float: left; margin-left: 30px;">
                                                <button style="background: #7bd1e0;" type="submit" name="medicine" class="input-regis">Submit</button>
                                            </div>
