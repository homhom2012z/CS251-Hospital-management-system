...
<?php
    ...
    $sql4 = "SELECT * FROM Dispense WHERE PatientID='$sql1'";
    $dispenseList = mysqli_query($conn, $sql4);

    $medQue = "SELECT * FROM Medicine";
    $medicines = mysqli_query($conn, $medQue);
    
    $medicinesData = $medicines->fetch_assoc();
        ...
        <div style="overflow:auto; padding: 2.5% 2.5% 2.5% 2.5%; ">
            ...
            <div class="main">
                ...
                <div class="card rounded-0 shadow-sm" style="width: 100%; float: left; padding: 0px;">
                    <div style="padding: 10px 20px 10px 20px; border-bottom: 1px solid rgba(0, 0, 0, 0.1);background-color: #42b7b9; color: white;"><h4>Medicine</h4></div>
                    <div style="min-height: 300px; overflow-y: hidden;">
                        <div style="margin-top: 0px; margin-bottom: 20px; margin-right: 20px; padding: 20px;">
                            ...
                            <div style="width: 100%; float: left; margin-bottom: 10px;">
                                <div style="width: 100%; font-weight: bold; text-align: center;">รายการสั่งยา</div>
                                ...
                                <div style="text-align: center;">
                                    ...
                                    <div style="width: 100%;padding : 20px; height:100%;">
                                        <?php
                                        $i=1;
                                        if(!$row = mysqli_fetch_array($dispenseList)){
                                        ?><div style="text-align: center;">ไม่พบประวัติการจ่ายยา</div>
                                        <?php    
                                        }
                                        while($row = mysqli_fetch_array($dispenseList))
                                        {
                                            ?>
                                            <div style="width:70%; float: left;">
                                            <?php
                                            $fetchx = $row['MedicineID'];

                                                $sql5 = "SELECT * FROM Medicine WHERE MedicineID='$fetchx'";
                                                $medicList = mysqli_query($conn, $sql5);
                                                $fetchxData = $medicList->fetch_assoc();
                                                echo $i.") ".$fetchxData['TradeName']." จำนวน : ".$fetchxData['Amount']." ราคา : ".$fetchxData['Price'];
                                                
                                            ?></div>
                                            <div style="width:25%; float: right; text-align: center;">
                                            <?php
                                            if($row['TrackingStatus']==='P'){
                                                echo "Preparing";
                                            }else if($row['TrackingStatus']==='S'){
                                                echo "Sending";
                                            }else if($row['TrackingStatus']==='D'){
                                                echo "Deilivered";
                                            }else{
                                                echo "Cancel";
                                            }
